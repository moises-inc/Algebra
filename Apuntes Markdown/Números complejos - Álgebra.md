--- 
title: Números complejos - Álgebra
origin: Notion
---
...
— $\sqrt[n]{z}$ representa a todos los números complejos $w\in \mathbb{C}$, tales que $w^{n} = z$. 

— Donde el teorema de *n*-ésimas de un complejo está dado:

$$w_k = |z|^{1/n} \cdot\text{cis} \left( \frac {\Phi + 2k\pi}{n} \right )$$

- Donde $\color{red}  k=n-1$.
- Por consecuencia del teorema, la suma de las soluciones debería ser igual a 0.
- La cantidad de soluciones dependerá del índice de la raíz
### Ejemplo: 

— Calcular $z² - \frac{1}{z} , z \ne 0$, para $z \in \mathbb{C}$.

$$\text{Bueno, } z² - \frac 1 z = 0 \\ \iff z³ - 1 = 0 \\ \iff z³ = 1 \\ \iff z =  \sqrt[3]{1}$$

$$\text{Luego, calcular las raíces de 1, donde } z = 1 + 0\cdot i \text{ así, } |z|= \sqrt{1²+0} = 1 \\ \text{además, el cis de } z =0$$

$$w_0 = 1 \cdot \text{cis} \left( \frac{0+0}{3}  \right ) = \text{cis}(0) = \cos 0 +  i\sin 0  = 1 + 0 = 1 \\ \therefore w_0 = 1 $$

$$w_1 = 1 \cdot \text{cis}\left(     \frac{0 + 2\pi}{3} \right) = \text{cis} \left(   \frac{2 \pi}{3}  \right) =  -\frac{1}{2} + \frac{\sqrt{3}}{2} i$$

$$w_2 = 1 \cdot \text{cis} \left (  \frac{0+4\pi}{3}  \right ) = \text{cis} \left(  \frac{4 \pi}{3} \right) = - \frac{1}{2} - \frac{\sqrt{3}}{2}i$$
